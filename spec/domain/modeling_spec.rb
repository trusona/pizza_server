...
RSpec.describe "Modeling" do
	...
	let(:get_pizza)       { Commands::GetPizza.new(repo: pizza_repo) }
	...
	let(:change_price)    { Commands::ChangePrice.new(repo: pizza_repo) }
	...
	def playground
		...
		pizza = change_price.call(pizza: pizza, new_price: 1.0)
		expect(get_orders.call.first.items.first.pizza.price).to be == 1
		expect(get_orders.call.first.items.first.price).to be == 4
